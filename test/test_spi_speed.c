...
int spi_speed_check (void)
{
  ...
  for (speed = 1 ; speed <= 32 ; speed *= 2)
  {
    ...
    spiFail = FALSE ;
    spiSetup (speed * 1000000) ;    //speed * 1MHz

    //memset(myData, speed, sizeof(myData));
...
int main (void)
{
  ...
  spiSetup(speed * 1000000) ;    //speed * 1MHz
  for (i=0; i<10; i++)
    wiringPiSPIDataRW(SPI_CHAN, &data[i], 1);
  close(myFd);

  speed=2;
  spiSetup(speed * 1000000) ;    //speed * 1MHz
  for (i=0; i<10; i++) data[i] = i+1;
  wiringPiSPIDataRW(SPI_CHAN, data, 10);
  close(myFd);
